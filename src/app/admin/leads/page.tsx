...
export default function AdminLeadsPage() {
  ...
  const filteredMsgs = messages.filter(m => {
    ...
    return (m.from || '').toLowerCase().includes(q) ||
      ...
      (m.listingtitle || '').toLowerCase().includes(q) ||
  ...
  const filteredUsers = users.filter(u => {
    if (!search) return true
    const q = search.toLowerCase()
    return `${u.fname} ${u.lname}`.toLowerCase().includes(q) ||
      (u.email || '').toLowerCase().includes(q)
  })

  const ROLE_COLORS: Record<string, { bg: string; color: string }> = {
    admin:    { bg: '#fef3dc', color: '#a86d1a' },
    agent:    { bg: '#e3f2fd', color: '#1a5ea8' },
    landlord: { bg: '#e1f5ee', color: '#2d7a4f' },
    buyer:    { bg: '#f0e8fd', color: '#6930c3' },
  }
    ...
    <>
      ...
      <div className="page-shell">
        <div className="page-header">
          <div>
            <h1 className="page-title">Leads & Users</h1>
            <p className="page-sub">{messages.length} messages · {users.length} registered users</p>
          </div>
        </div>
        ...
        {loading ? (
                  ...
                  <div className="msg-top">
                    <div>
                      <div className="msg-tag">
                        {m.listingtitle ? `📋 ${m.listingtitle}` : '📋 General'}
                        <span className={`msg-type ${m.type === 'viewing' ? 'type-viewing' : 'type-msg'}`}>
                          {m.type === 'viewing' ? '🗓 Viewing Request' : '💬 Enquiry'}
                        </span>
                      </div>
                    ...
                    <div className="msg-actions">
                      ...
                      {m.listingid && (
                        <button className="btn btn-sm" onClick={() => router.push(`/property/${m.listingid}`)}>View Property</button>
                      )}
          ...
          filteredUsers.length === 0 ? (
            <div className="empty-msg">No registered users yet.</div>
          ) : (
                  ...
                  {filteredUsers.map(u => {
                    ...
                    return (
                      <tr key={u.id}>
                        ...
                        <td>
                          <span className="pill" style={{ background: rc.bg, color: rc.color }}>
                            {u.role || 'buyer'}
                          </span>
                        </td>
