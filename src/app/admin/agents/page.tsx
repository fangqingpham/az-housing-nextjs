  ...
  return (
    <>
      ...
      <div className="page-shell">
        <div className="page-header">
          <div>
            ...
            <p className="page-sub">{users.length} total registered accounts</p>
          </div>
        </div>
        ...
        <div className="role-summary">
          {ROLES.map(r => {
            ...
            return (
              <button
                ...
                className="role-chip"
                style={{
                  background: roleFilter === r ? rc.color : rc.bg,
                  color: roleFilter === r ? '#fff' : rc.color,
          ...
          {roleFilter && (
            <button className="role-chip-clear" onClick={() => setRoleFilter('')}>✕ Clear filter</button>
          )}
        </div>
        ...
        {loading ? (
          <div className="empty-msg">Loading users…</div>
        ) : filtered.length === 0 ? (
          <div className="empty-msg">No users match your filters.</div>
        ) : (
                    ...
                    <tr key={u.id}>
                      <td>
                        <div className="user-avatar">
                          <div className="avatar-circle" style={{ background: rc.bg, color: rc.color }}>
                            {(u.fname?.[0] || '?').toUpperCase()}
                          </div>
                          <span className="td-name">{u.fname} {u.lname || ''}</span>
                        </div>
                      </td>
                      <td>
                        <a href={`mailto:${u.email}`} className="td-email">{u.email}</a>
                      </td>
      ...
      <style jsx>{`
        ...
        .role-select {
          border:1px solid; border-radius:7px; padding:4px 8px; font-size:12px; font-weight:700;
          cursor:pointer; outline:none; font-family:inherit; transition:opacity 0.15s;
        }
