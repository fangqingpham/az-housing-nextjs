...
export default function AdminOrdersPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    const r = await fetch('/api/admin/tenant-placement-orders', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) })
  ...
  const filtered = orders.filter(o => {
    ...
    if (search) {
      ...
      if (![o.landlord_name, o.company_name||'', o.phone, o.email, o.property_address, o.city, o.postal_code].join(' ').toLowerCase().includes(q)) return false
    }
    return true
  })
    ...
    <>
      ...
      <div className="page-shell">
        <div className="page-header">
          <div><h1 className="page-title">Tenant Orders</h1><p className="page-sub">{orders.length} total · {orders.filter(o=>o.status==='new').length} new</p></div>
        ...
        {loading ? <div className="empty-msg">Loading orders…</div> : filtered.length === 0 ? <div className="empty-msg">No orders match your filters.</div> : (
                ...
                <div key={o.id} className="order-card">
                  <div className="order-card-top" onClick={() => setExpanded(isOpen ? null : o.id)}>
                    <div className="order-meta">
                      ...
                      <div className="order-date">Submitted {o.created_at ? new Date(o.created_at).toLocaleDateString('en-CA') : '--'}</div>
                    </div>
                  ...
                  {isOpen && (
                    <div className="order-detail">
                      ...
                      {Array.isArray(o.selected_services)&&o.selected_services.length>0&&(
                        <div className="services-block"><strong>Selected Services</strong><ul>{o.selected_services.map((s,i)=><li key={i}>{s}</li>)}</ul></div>
                      )}
                      {o.additional_notes&&<div className="notes-block"><strong>Notes</strong><p>{o.additional_notes}</p></div>}
