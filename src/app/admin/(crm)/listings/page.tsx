  ...
  return (
    <>
      ...
      <div className="page-shell">
        ...
        {loading ? <div className="empty-msg">Loading…</div> : filtered.length === 0 ? <div className="empty-msg">No listings match your filters.</div> : (
          <div className="table-wrap">
            <table className="crm-table">
              <thead><tr><th></th><th>Title</th><th>Price</th><th>Type</th><th>City</th><th>Beds</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
              <tbody>
                {filtered.map(l => {
                  ...
                  return (
                    <tr key={l.id}>
                      <td><div className="thumb">{imgs[0] ? <img src={imgs[0]} alt="" /> : <span style={{ color:'#f5a623',opacity:0.4,fontSize:20 }}>🏠</span>}</div></td>
                      <td><div className="td-name">{l.title}</div><div className="td-sub">{l.ptype}</div></td>
                      ...
                      <td>
                        <div className="action-row">
                          ...
                          {status === 'pending' && <button className="btn btn-sm btn-approve" onClick={() => approve(l.id)}>Approve</button>}
                          {l.author !== 'seed' && <button className="btn btn-sm btn-danger" onClick={() => remove(l.id)}>Delete</button>}
                        </div>
                      </td>
                    </tr>
                  )
