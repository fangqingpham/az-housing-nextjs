...
type Transaction = { id: string; created_at: string; date: string; description: string; amount: number; type: TxType; category: TxCategory; notes: string | null; client_name: string | null; property_address: string | null }

const CATEGORIES: TxCategory[] = ['commission', 'placement fee', 'management fee', 'referral', 'marketing', 'operations', 'other']
const MONTH_NAMES = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
const emptyTx = (): Omit<Transaction, 'id' | 'created_at'> => ({ date: new Date().toISOString().slice(0, 10), description: '', amount: 0, type: 'income', category: 'commission', notes: '', client_name: '', property_address: '' })
...
export default function AdminFinancesPage() {
  ...
  const save = async () => {
    ...
    const { error } = modal.isNew ? await supabase.from('transactions').insert([payload]) : await supabase.from('transactions').update(payload).eq('id', modal.tx!.id)
    if (error) { showToast('Failed to save.'); setSaving(false); return }
    showToast(modal.isNew ? 'Transaction added ✓' : 'Transaction updated ✓')
  ...
  const remove = async (id: string) => {
    ...
    setTxs(prev => prev.filter(t => t.id !== id)); showToast('Transaction deleted.')
  }
  ...
  const monthlyData = MONTH_NAMES.map((_, i) => {
    ...
    const mt = txs.filter(t => t.date?.slice(0, 4) === yearFilter && t.date?.slice(5, 7) === month)
    return { label: MONTH_NAMES[i], income: mt.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0), expense: mt.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0) }
  })
    ...
    <>
      ...
      {modal.open && (
        <div className="modal-overlay" onClick={() => setModal({ open: false, isNew: true })}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="fr">
              <div className="fg"><label>Type</label><select className="fc" value={form.type} onChange={f('type')}><option value="income">Income</option><option value="expense">Expense</option></select></div>
              <div className="fg"><label>Category</label><select className="fc" value={form.category} onChange={f('category')}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></div>
            </div>
      ...
      <div className="page-shell">
        <div className="page-header">
          <div><h1 className="page-title">Finances</h1><p className="page-sub">Income, expenses, and net revenue</p></div>
          <button className="btn-accent-pill" onClick={() => { setForm(emptyTx()); setModal({ open: true, isNew: true }) }}>+ Add Transaction</button>
        </div>
        <div className="summary-grid">
          <div className="summary-card"><div className="summary-label">Total Income</div><div className="summary-val" style={{color:'#2d7a4f'}}>${totalIncome.toLocaleString('en-CA',{minimumFractionDigits:2})}</div></div>
          <div className="summary-card"><div className="summary-label">Total Expenses</div><div className="summary-val" style={{color:'#a32d2d'}}>${totalExpense.toLocaleString('en-CA',{minimumFractionDigits:2})}</div></div>
          <div className="summary-card summary-card--net"><div className="summary-label">Net Revenue</div><div className="summary-val" style={{color:net>=0?'#2d7a4f':'#a32d2d'}}>{net>=0?'':'-'}${Math.abs(net).toLocaleString('en-CA',{minimumFractionDigits:2})}</div></div>
        </div>
        <div className="chart-wrap">
          ...
          <div className="chart-legend"><span className="legend-dot" style={{background:'#2d7a4f'}} /> Income<span className="legend-dot" style={{background:'#a32d2d',marginLeft:12}} /> Expense</div>
        </div>
        <div className="filter-bar">
          <select className="fc" style={{maxWidth:150,marginBottom:0}} value={typeFilter} onChange={e=>setTypeFilter(e.target.value as any)}><option value="">All types</option><option value="income">Income</option><option value="expense">Expense</option></select>
          <select className="fc" style={{maxWidth:180,marginBottom:0}} value={catFilter} onChange={e=>setCatFilter(e.target.value as any)}><option value="">All categories</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select>
        </div>
        {loading ? <div className="empty-msg">Loading transactions…</div> : filtered.length===0 ? <div className="empty-msg">No transactions yet. Add your first one above.</div> : (
          <div className="table-wrap">
            <table className="crm-table">
              <thead><tr><th>Date</th><th>Description</th><th>Category</th><th>Client</th><th>Type</th><th>Amount</th><th></th></tr></thead>
              <tbody>
                {filtered.map(t=>(
                  <tr key={t.id}>
                    <td className="td-sub td-nowrap">{t.date}</td>
                    <td><div className="td-name">{t.description}</div>{t.property_address&&<div className="td-sub">{t.property_address}</div>}</td>
                    <td><span className="cat-pill">{t.category}</span></td>
                    <td className="td-sub">{t.client_name||'--'}</td>
                    <td><span className={`type-pill ${t.type==='income'?'pill-income':'pill-expense'}`}>{t.type==='income'?'▲':'▼'} {t.type}</span></td>
                    <td className={`td-amount ${t.type==='income'?'amount-in':'amount-out'}`}>{t.type==='income'?'+':'-'}${Number(t.amount).toLocaleString('en-CA',{minimumFractionDigits:2})}</td>
                    <td><div className="action-row"><button className="btn btn-sm" onClick={()=>{setForm({...t});setModal({open:true,isNew:false,tx:t})}}>Edit</button><button className="btn btn-sm btn-danger" onClick={()=>remove(t.id)}>Del</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
