...
type Transaction = {
  id: string; created_at: string; date: string; description: string
  amount: number; type: string; category: string; notes: string | null
  client_name: string | null; property_address: string | null
}
...
export default function AdminCommissionPage() {
  ...
  const [form, setForm]     = useState({ date: new Date().toISOString().slice(0,10), description: '', amount: 0, client_name: '', property_address: '', notes: '' })
  ...
  const save = async () => {
    ...
    const payload = { ...form, amount: Number(form.amount), type: 'expense', category: 'commission' }
    const { error } = modal.tx
      ? await supabase.from('transactions').update(payload).eq('id', modal.tx.id)
      : await supabase.from('transactions').insert([payload])
    if (error) { showToast('Failed to save.'); setSaving(false); return }
    showToast(modal.tx ? 'Updated ✓' : 'Added ✓')
    setModal({ open: false })
    await load()
    setSaving(false)
  }
  ...
  const openEdit = (tx: Transaction) => {
    setForm({ date: tx.date, description: tx.description, amount: tx.amount, client_name: tx.client_name || '', property_address: tx.property_address || '', notes: tx.notes || '' })
    setModal({ open: true, tx })
  }

  const openNew = () => {
    setForm({ date: new Date().toISOString().slice(0,10), description: '', amount: 0, client_name: '', property_address: '', notes: '' })
    setModal({ open: true })
  }

  const years = Array.from(new Set(txs.map(t => t.date?.slice(0,4)).filter(Boolean))).sort().reverse()
  if (years.length && !years.includes(yearFilter)) setYearFilter(years[0])

  const yearTxs = txs.filter(t => t.date?.slice(0,4) === yearFilter)
  const filtered = yearTxs.filter(t => {
    if (!search) return true
    const q = search.toLowerCase()
    return [t.description, t.client_name||'', t.property_address||''].join(' ').toLowerCase().includes(q)
  })

  const totalCommission = yearTxs.filter(t => t.type === 'expense').reduce((s,t) => s + t.amount, 0)
  const totalIncome     = yearTxs.filter(t => t.type === 'income').reduce((s,t) => s + t.amount, 0)
  const net             = totalIncome - totalCommission

  const monthlyData = MONTH_NAMES.map((_, i) => {
    const month = String(i+1).padStart(2,'0')
    const mt = yearTxs.filter(t => t.date?.slice(5,7) === month)
    return {
      label: MONTH_NAMES[i],
      commission: mt.filter(t => t.type === 'expense').reduce((s,t) => s + t.amount, 0),
      income:     mt.filter(t => t.type === 'income').reduce((s,t) => s + t.amount, 0),
    }
  })
  const maxMonthly = Math.max(...monthlyData.map(m => Math.max(m.commission, m.income)), 1)
  ...
  return (
    <>
      ...
      {modal.open && (
        <div className="modal-overlay" onClick={() => setModal({ open: false })}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="modal-title">{modal.tx ? 'Edit Commission Entry' : 'Add Commission Entry'}</h3>
            <div className="fr">
            ...
            <div className="fr">
              <div className="fg"><label>Agent / Client</label><input className="fc" value={form.client_name} onChange={f('client_name')} /></div>
            ...
            <div className="modal-actions">
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving ? 'Saving…' : modal.tx ? 'Save Changes' : 'Add Entry'}</button>
              <button className="btn" onClick={() => setModal({ open: false })}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="page-shell">
        <div className="page-header">
          <div>
            ...
            <p className="page-sub">Agent commission tracking — auto-updated from closed deals</p>
          </div>
          <button className="btn-accent-pill" onClick={openNew}>+ Add Entry</button>
        </div>
        ...
        <div className="summary-grid">
          <div className="summary-card">
            ...
            <div className="summary-val" style={{ color: '#a32d2d' }}>${totalCommission.toLocaleString('en-CA', { minimumFractionDigits: 2 })}</div>
          </div>
          <div className="summary-card">
            <div className="summary-label">Total Deal Revenue</div>
          ...
          <div className="summary-card summary-card--net">
            <div className="summary-label">Net After Commission</div>
        ...
        <div className="chart-wrap">
          <div className="chart-header">
            <span className="chart-title">Monthly Commission vs Revenue</span>
            ...
            {monthlyData.map(m => (
              <div key={m.label} className="chart-col">
                <div className="bar-pair">
                  <div className="bar bar-income"  style={{ height: `${(m.income/maxMonthly)*100}%` }}     title={`Revenue: $${m.income.toLocaleString('en-CA')}`} />
                  <div className="bar bar-expense" style={{ height: `${(m.commission/maxMonthly)*100}%` }} title={`Commission: $${m.commission.toLocaleString('en-CA')}`} />
                </div>
          ...
          <div className="chart-legend">
            <span className="legend-dot" style={{ background: '#2d7a4f' }} /> Revenue
            <span className="legend-dot" style={{ background: '#a32d2d', marginLeft: 12 }} /> Commission
          </div>
        </div>

        {/* Filters */}
        <div className="filter-bar">
          <input className="fc" style={{ maxWidth: 280, marginBottom: 0 }} placeholder="Search agent, property, description…" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-msg">No commission entries yet. They'll appear automatically when deals close.</div>
        ) : (
          <div className="table-wrap">
            <table className="crm-table">
              <thead>
                <tr>
                  <th>Date</th><th>Description</th><th>Agent / Client</th><th>Property</th><th>Type</th><th>Amount</th><th></th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(t => (
                  <tr key={t.id}>
                    ...
                    <td>
                      <div className="td-name">
                        {t.description}
                        {t.notes?.startsWith('Auto-generated') && <span className="auto-badge">Auto</span>}
                      </div>
                    </td>
                    <td className="td-sub">{t.client_name || '--'}</td>
                    <td className="td-sub">{t.property_address || '--'}</td>
                    <td>
                      <span className={`type-pill ${t.type === 'income' ? 'pill-income' : 'pill-expense'}`}>
                        {t.type === 'income' ? '▲ Revenue' : '▼ Commission'}
                      </span>
                    </td>
      ...
      <style jsx>{`
        ...
        .empty-msg{text-align:center;color:#a8a8a4;padding:48px;background:#fff;border-radius:12px;border:1px solid #e4e1d8;font-size:14px;line-height:1.6}
        ...
        .td-name{font-weight:600;color:#1b2a4a;display:flex;align-items:center;gap:6px;flex-wrap:wrap}
        ...
        .auto-badge{display:inline-block;background:#e3f2fd;color:#1a5ea8;border-radius:4px;padding:1px 6px;font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase}
        ...
        .modal-box{background:#fff;border-radius:18px;padding:32px 28px;max-width:540px;width:100%;max-height:90vh;overflow-y:auto;position:relative}
