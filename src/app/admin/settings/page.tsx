...
const emptyArticle = (): BlogPost => ({
  id: '', title: '', cat: 'Buying Guide', excerpt: '',
  date: new Date().toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' }),
  read: '5 min read', color: '#E8F4FD', body: '', image: '', author: '',
})
...
export default function AdminSettingsPage() {
  ...
  // Site settings (stored via /api/settings)
  ...
  // Lazy-load articles when tab becomes active
  useEffect(() => {
  ...
  const saveSettings = async () => {
    ...
    try {
      const r = await fetch('/api/settings', {
        ...
        body: JSON.stringify({ hero: heroText, herosub: heroSub }),
      })
  ...
  const openNewArticle = () => { setArtForm(emptyArticle()); setArtBodyMode('write'); setArtModal({ open: true, isNew: true }) }
  const openEditArticle = (a: BlogPost) => { setArtForm({ ...a }); setArtBodyMode('write'); setArtModal({ open: true, isNew: false }) }
  ...
  const uploadImage = async (file: File) => {
    ...
    setArtField('image', data.publicUrl)
  ...
  const saveArticle = async () => {
    ...
    setArtModal({ open: false, isNew: true })
    await loadArticles()
  }
      ...
      {artModal.open && (
        <div className="modal-overlay" onClick={() => setArtModal({ open: false, isNew: true })}>
          <div className="modal-box wide-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="fg"><label>Title *</label><input className="fc" placeholder="Article headline…" value={artForm.title} onChange={e => setArtField('title', e.target.value)} /></div>
            <div className="fr">
              ...
              <div className="fg"><label>Read Time</label><input className="fc" placeholder="5 min read" value={artForm.read} onChange={e => setArtField('read', e.target.value)} /></div>
            </div>
            <div className="fr">
              <div className="fg"><label>Author</label><input className="fc" placeholder="Author name" value={artForm.author || ''} onChange={e => setArtField('author', e.target.value)} /></div>
              <div className="fg"><label>Date</label><input className="fc" placeholder="May 22, 2026" value={artForm.date} onChange={e => setArtField('date', e.target.value)} /></div>
            </div>
            ...
            <div className="fg">
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <label className="upload-btn">
                  {imgUploading ? '⏳' : '📁 Upload'}
                  <input type="file" accept="image/*" style={{ display: 'none' }} disabled={imgUploading} onChange={e => { const f = e.target.files?.[0]; if (f) uploadImage(f); e.target.value = '' }} />
                </label>
              </div>
              {artForm.image && (
                <div style={{ marginTop: 8, borderRadius: 8, overflow: 'hidden', height: 100, position: 'relative' }}>
                  <img src={artForm.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <button onClick={() => setArtField('image', '')} style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(0,0,0,0.5)', color: '#fff', border: 'none', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', fontSize: 12 }}>✕</button>
                </div>
              )}
            </div>
            ...
            <div className="fg">
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                {ARTICLE_COLORS.map(c => (
                  <button key={c} onClick={() => setArtField('color', c)} style={{ width: 26, height: 26, borderRadius: '50%', background: c, border: 'none', cursor: 'pointer', outline: artForm.color === c ? '2px solid #f5a623' : '2px solid transparent', outlineOffset: 2 }} />
                ))}
              </div>
            </div>

            <div className="fg"><label>Excerpt *</label><textarea className="fc" rows={2} placeholder="Short summary for the blog card…" value={artForm.excerpt} onChange={e => setArtField('excerpt', e.target.value)} /></div>
            ...
            <div className="fg">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {(['write', 'preview'] as const).map(m => (
                    <button key={m} onClick={() => setArtBodyMode(m)} style={{ fontSize: 12, padding: '3px 10px', borderRadius: 6, cursor: 'pointer', background: artBodyMode === m ? '#f5a623' : 'transparent', color: artBodyMode === m ? '#1e2a45' : '#a8a8a4', border: '1px solid #e4e1d8', fontFamily: 'inherit' }}>
              ...
              {artBodyMode === 'write' ? (
                <textarea className="fc" rows={9} placeholder="<p>Your article body…</p>" value={artForm.body} onChange={e => setArtField('body', e.target.value)} style={{ fontFamily: 'monospace', fontSize: 13 }} />
              ) : (
                <div style={{ border: '1px solid #e4e1d8', borderRadius: 8, padding: '1rem', minHeight: 160, background: '#fafaf8', fontSize: 14, lineHeight: 1.8, color: '#6b6b67' }} dangerouslySetInnerHTML={{ __html: artForm.body || '<p style="color:#ccc;font-style:italic">Nothing to preview yet.</p>' }} />
              )}
            </div>
        ...
        {tab === 'site' && (
          <div className="settings-section">
            <div className="settings-card">
              ...
              <div className="fg"><label>Hero Title</label><input className="fc" placeholder="Find Your Perfect Home Across Canada" value={heroText} onChange={e => setHeroText(e.target.value)} /></div>
              <div className="fg"><label>Hero Subtitle</label><input className="fc" placeholder="Browse thousands of listings…" value={heroSub} onChange={e => setHeroSub(e.target.value)} /></div>
        ...
        {tab === 'articles' && (
          <>
            <div className="filter-bar">
              ...
              <select className="fc" style={{ maxWidth: 200, marginBottom: 0 }} value={artCat} onChange={e => setArtCat(e.target.value)}>
                <option value="">All categories</option>
                {ARTICLE_CATEGORIES.map(c => <option key={c}>{c}</option>)}
              </select>
              <button className="btn btn-primary" style={{ marginLeft: 'auto' }} onClick={openNewArticle}>+ New Article</button>
            </div>

            {artLoading ? (
              <div className="empty-msg">Loading articles…</div>
            ) : filteredArticles.length === 0 ? (
              <div className="empty-msg">
                <div style={{ fontSize: 36, marginBottom: 12 }}>✍️</div>
                <p style={{ fontWeight: 600, marginBottom: 6 }}>No articles yet</p>
                <p style={{ color: '#a8a8a4', fontSize: 13, marginBottom: 18 }}>Articles you publish here appear on the blog alongside the built-in posts.</p>
                <button className="btn btn-primary" onClick={openNewArticle}>Write Your First Article</button>
              </div>
            ) : (
              <div className="table-wrap">
                <table className="crm-table">
                  <thead><tr><th></th><th>Title</th><th>Category</th><th>Author</th><th>Read Time</th><th>Date</th><th>Actions</th></tr></thead>
                  <tbody>
                    {filteredArticles.map(a => (
                      <tr key={a.id}>
                        <td>
                          <div style={{ width: 38, height: 38, borderRadius: 7, background: a.image ? `url(${a.image}) center/cover no-repeat` : (a.color || '#E8F4FD'), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
                            {!a.image && '📄'}
                          </div>
                        </td>
                        <td>
                          <div className="td-name" style={{ maxWidth: 260 }}>{a.title}</div>
                          <div className="td-sub" style={{ maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.excerpt}</div>
                        </td>
                        ...
                        <td>
                          <div className="action-row">
                            ...
                            <button className="btn btn-sm btn-danger" onClick={() => removeArticle(a.id)}>Delete</button>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        {tab === 'account' && (
          <div className="settings-section">
            <div className="settings-card">
              ...
              <button className="btn btn-primary" onClick={changePassword} disabled={updatingPass}>
            ...
            <div className="settings-card">
              ...
              <p className="settings-hint">Sign out of the admin portal. You will be redirected to the login page.</p>
              <button className="btn btn-danger-solid" onClick={async () => {
                const supabase = createClient()
