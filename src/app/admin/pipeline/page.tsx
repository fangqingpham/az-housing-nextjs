...
const empty = (): Omit<Deal, 'id' | 'created_at'> => ({
  ...
  deal_type: 'buy', value: null, stage: 'lead', notes: '', assigned_agent: '',
})
...
export default function AdminPipelinePage() {
  ...
  const openNew = () => { setForm(empty()); setModal({ open: true, isNew: true }) }
  const openEdit = (d: Deal) => { setForm({ ...d }); setModal({ open: true, isNew: false, deal: d }) }

  const save = async () => {
    if (!form.client_name.trim() || !form.property_address.trim()) {
      showToast('Client name and property are required.'); return
    }
    setSaving(true)
    const supabase = createClient()
    if (modal.isNew) {
      const { error } = await supabase.from('deals').insert([form])
      if (error) { showToast('Failed to create deal.'); setSaving(false); return }
      showToast('Deal created ✓')
    } else {
      const { error } = await supabase.from('deals').update(form).eq('id', modal.deal!.id)
      if (error) { showToast('Failed to update deal.'); setSaving(false); return }
      ...
      {modal.open && (
        <div className="modal-overlay" onClick={() => setModal({ open: false, isNew: true })}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="fr">
              <div className="fg"><label>Deal Value ($)</label><input className="fc" type="number" placeholder="e.g. 650000" value={form.value ?? ''} onChange={e => setForm(p => ({ ...p, value: e.target.value ? Number(e.target.value) : null }))} /></div>
            ...
            <div className="fg"><label>Assigned Agent</label><input className="fc" placeholder="Agent name" value={form.assigned_agent || ''} onChange={f('assigned_agent')} /></div>
      ...
      <div className="page-shell">
        <div className="page-header">
          <div>
            ...
            <p className="page-sub">{deals.length} active deals · ${totalValue.toLocaleString('en-CA')} closed value</p>
          </div>
          <button className="btn-accent-pill" onClick={openNew}>+ New Deal</button>
        </div>

        {loading ? (
          <div className="empty-msg">Loading pipeline…</div>
        ) : (
            ...
            {STAGES.map(stage => {
              ...
              return (
                <div key={stage.key} className="kanban-col">
                  ...
                  {colValue > 0 && (
                    <div className="col-value">${colValue.toLocaleString('en-CA')}</div>
                  )}
                  <div className="col-cards">
                    {cols.length === 0 && <div className="col-empty">No deals</div>}
                    {cols.map(d => (
                      <div key={d.id} className="deal-card">
                        <div className="deal-type-tag" style={{ background: stage.bg, color: stage.color }}>
                          {TYPE_LABELS[d.deal_type]}
                        </div>
