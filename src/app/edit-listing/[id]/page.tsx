...
export default function EditListingPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      router.replace(`/auth/login?redirect=/edit-listing/${listingId}`)
    }
  ...
  useEffect(() => {
    const loadListing = async () => {
      ...
      if (String(listing.author) !== String(user.id)) {
        showToast('You do not have permission to edit this listing.')
        router.push('/dashboard')
        return
      }
      ...
      const featureList = listing.feats || listing.features || []
      setFeats(Array.isArray(featureList) ? featureList.join('\n') : '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      showToast('Please log in before saving.')
      return
    }
