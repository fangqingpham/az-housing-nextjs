...
const SERVICES = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'Custom lease review & advice',
      'Addendum creation (pets, parking, etc.)',
      'Digital signing integration',
      'Secure cloud storage of agreements',
    ],
  ...
  },
  {
    ...
    body: [
      'From rent collection to maintenance coordination, A-Z Housing's property management partners handle every aspect of running your rental. Whether you own a single condo or a multi-unit portfolio, we match you with accredited local property managers who know your market.',
      'Our dashboard lets you track rent payments, maintenance tickets, and tenancy status in real time — even when you're managing from out of province or overseas.',
    ],
    bullets: [
      'Rent collection & arrears management',
      'Maintenance coordination & contractor network',
      'Regular property inspections with photo reports',
      'Month-end financial statements',
      'Portfolio dashboard (single & multi-unit)',
    ],
  ...
  },
  {
    ...
    body: [
      ...
      'We don't replace formal legal counsel — we help you understand the process so you arrive prepared. Most initial consultations are offered at a flat rate through our platform.',
    ],
    bullets: [
      'Notice preparation (N4, N5, N12, N13 & equivalents)',
      'LTB application guidance',
      'Hearing preparation & documentation checklist',
      'Referrals to licensed paralegals & real estate lawyers',
    ...
    <main style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--dark) 0%, #1a2a4a 100%)', color: '#fff', padding: 'clamp(60px,10vw,110px) 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            {SERVICES.map(s => (
              <a key={s.id} href={`#${s.id}`} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', borderRadius: 8, padding: '9px 18px', fontSize: 13, textDecoration: 'none', fontWeight: 500, transition: 'background .2s' }}
      ...
      {SERVICES.map((s, i) => (
        <section
          key={s.id}
          id={s.id}
          style={{ padding: 'clamp(60px,8vw,100px) 24px', background: i % 2 === 0 ? 'var(--cream)' : '#fff', scrollMarginTop: 80 }}
        >
          <div style={{ maxWidth: 1060, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px,1fr))', gap: 56, alignItems: 'center' }}>
            <div style={{ order: i % 2 === 0 ? 0 : 1 }}>
              ...
              <div style={{ display: 'inline-block', background: s.accent, color: '#fff', fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', borderRadius: 20, padding: '4px 14px', marginBottom: 16 }}>
                Landlord Service
              </div>
              <h2 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(1.6rem,3.5vw,2.4rem)', color: 'var(--dark)', marginBottom: 10, lineHeight: 1.25 }}>
                {s.title}
              </h2>
              <p style={{ color: s.accent, fontWeight: 600, marginBottom: 20, fontSize: '1.05rem' }}>{s.tagline}</p>
              {s.body.map((para, j) => (
                <p key={j} style={{ color: 'var(--mid)', lineHeight: 1.8, marginBottom: 16 }}>{para}</p>
              ))}
              <Link href={s.cta.href}
                style={{ display: 'inline-block', background: 'var(--dark)', color: '#fff', textDecoration: 'none', borderRadius: 10, padding: '12px 28px', fontWeight: 700, fontSize: 14, marginTop: 8, transition: 'opacity .2s' }}
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {s.bullets.map(b => (
                    <li key={b} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, color: 'var(--mid)', fontSize: 14, lineHeight: 1.5 }}>
                      <span style={{ color: s.accent, fontWeight: 800, fontSize: 16, flexShrink: 0, marginTop: 1 }}>✓</span>
                      {b}
                    </li>
                  ))}
                </ul>
      ...
      <section style={{ background: 'var(--accent)', padding: 'clamp(50px,7vw,80px) 24px', textAlign: 'center', color: '#fff' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.82)', marginBottom: 32, maxWidth: 480, margin: '0 auto 32px', fontSize: '1.05rem' }}>
          List your property for free and access all landlord tools from your dashboard.
        </p>
