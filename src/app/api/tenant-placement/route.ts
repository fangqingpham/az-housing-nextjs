...
async function sendNewOrderEmail(order: {
  ...
  const rows = [
    ...
    ['Bedrooms / Baths',    `${order.bedrooms || '—'} / ${order.bathrooms || '—'}`],
    ...
    .map(([lbl, val], i) => `
      <tr style="background:${i % 2 === 0 ? '#fff' : '#f9f7f4'};">
        <td style="padding:12px 18px;font-weight:700;color:#1e2a45;font-size:13px;white-space:nowrap;width:140px;">${lbl}</td>
  ...
  await getTransporter().sendMail({
    ...
    to: 'info@azhouse.ca',
    replyTo: order.email,
    subject: `📋 New Tenant Placement Order — ${order.landlordName} · ${order.propertyAddress}, ${order.city}`,
    text: [
      'A new Tenant Placement order has been submitted via the website.',
      '',
      ...
      '',
      `Order ID: ${order.id}`,
      'Log in to the CRM to view and assign this order.',
    ].join('\n'),
    html: `
      <div style="font-family:Arial,sans-serif;max-width:650px;margin:0 auto;background:#f7f4ef;padding:32px 24px;border-radius:12px;">
        <div style="background:#1e2a45;border-radius:10px;padding:22px 28px;margin-bottom:24px;">
          ...
          <p style="color:rgba(255,255,255,0.65);margin:0;font-size:13px;">
            Submitted via the A-Z Housing website — please log in to assign an agent.
          </p>
        </div>
        ...
        <div style="background:#1e2a45;border-radius:10px;padding:16px 22px;text-align:center;">
          <p style="color:rgba(255,255,255,0.6);font-size:13px;margin:0 0 10px;">Estimated Total</p>
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        authorization_confirmed: Boolean(form.authorization || form.authorizationConfirmed || body.authorizationConfirmed || body.authorization_confirmed),
        status: 'new',
      })
      .select('id')
      .single();
    ...
    }).catch(err => console.error('[tenant-placement] Email notification failed:', err));
