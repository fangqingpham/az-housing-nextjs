...
import { getListings, getUserCount, getSavedIds, toggleSaved } from '@/lib/api';
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* ── Featured For Sale (hidden temporarily) ── */}
      {false && (
      ...
      {/* ── Featured For Rent (hidden temporarily) ── */}
      {false && (
        <section className="sec">
          <div className="container">
            <div className="sec-hdr">
      ...
      {/* ── Services band ── */}
      <section style={{ background: '#f7f4ef', padding: 'clamp(48px,7vw,80px) 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <span style={{ fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', color: 'var(--accent)', fontWeight: 600 }}>
            Our Services
          </span>
          <h2 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(1.6rem,3vw,2.4rem)', marginBottom: 12, color: 'var(--dark)' }}>
            Everything You Need Under One Roof
          </h2>
          <p style={{ color: 'var(--mid)', maxWidth: 540, margin: '0 auto 40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 24 }}>
            {[
              ...
              <Link key={s.title} href={s.href} style={{ textDecoration: 'none' }}>
                <div
                  style={{ background: '#fff', borderRadius: 14, padding: '28px 22px', border: '1px solid rgba(0,0,0,0.07)', transition: 'transform .18s, box-shadow .18s', cursor: 'pointer' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 10px 32px rgba(0,0,0,0.10)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}
                >
                  <div style={{ fontSize: 32, marginBottom: 14 }}>{s.icon}</div>
                  <h3 style={{ fontFamily: 'var(--serif)', fontSize: '1.1rem', color: 'var(--dark)', marginBottom: 8 }}>{s.title}</h3>
